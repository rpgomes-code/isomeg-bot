...
export class KickCommand extends Command {
    ...
    public override async chatInputRun(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const check = checkModPermissions(moderator, target, bot, PermissionFlagsBits.KickMembers);
        const failure = getModPermissionFailure(check, "kick");
        if (failure) {
        ...
        await logModAction(interaction.guild, "Kick", target, moderator, reason);
    ...
    public override async messageRun(message: Message, args: any): Promise<void> {
        ...
        if (!target || !message.guild) {
            await message.reply("Please mention a user to kick. e.g. `$kick @User [reason]`");
            return;
        }
        ...
        try {
            ...
            const check = checkModPermissions(moderator, member, bot, PermissionFlagsBits.KickMembers);
            const failure = getModPermissionFailure(check, "kick");
            if (failure) {
            ...
            await logModAction(message.guild, "Kick", member, moderator, reason);
            await message.reply(`${target.tag} has been kicked.`);
        } catch {
            await message.reply(`Failed to kick ${target.tag}.`);
        }
    }
}
