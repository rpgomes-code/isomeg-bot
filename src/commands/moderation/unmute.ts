...
export class UnmuteCommand extends Command {
    ...
    public override async chatInputRun(interaction: ChatInputCommandInteraction): Promise<void> {
        ...
        const failure = getModPermissionFailure(check, "unmute");
        if (failure) {
        ...
        await logModAction(interaction.guild, "Unmute", target, moderator, reason);
    ...
    public override async messageRun(message: Message, args: any): Promise<void> {
        ...
        if (!target || !message.guild) {
            await message.reply("Please mention a user to unmute. e.g. `$unmute @User`");
            return;
        }
        ...
        try {
            ...
            await logModAction(message.guild, "Unmute", member, moderator, reason);
            await message.reply(`${target.tag} has been unmuted.`);
        } catch {
            await message.reply(`Failed to unmute ${target.tag}.`);
        }
    }
}
